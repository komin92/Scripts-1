...
$GroupList = Get-ADGroup -SearchBase $SearchBase -Filter {Name -like $ADFilter } |`
    ...
    $AddGroup = $GroupList | Out-GridView -Title 'Group to be Removed' -PassThru
    ...
    Foreach ($User in $Process) {
        if($Process[$i] -match '.1877' -and $AddGroup -match '.*WksLocalAdmin'){
            ...
            if(Get-ADGroupMember -Identity $AddGroup | Where-Object Name -like "*$($Process[$i])*"){
                Write-Output "Attempting to remove $AddGroup from $Process[$i]"
                Try{
                    Remove-ADGroupMember -Identity $AddGroup -Members $($Process[$i])
                }Catch{
                    Write-Warning "Failed to Remove $AddGroup from $Process[$i]"
                }
                $i = $i + 1
            }Else{
